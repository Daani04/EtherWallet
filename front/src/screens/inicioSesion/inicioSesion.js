...
const InicioSesion = (props) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      Alert.alert(response.ok ? t("login.biometric.successTitle") : t("login.alerts.errorTitle"), text);
      ...
      if (!fetchedId) {
        Alert.alert(t("login.alerts.errorTitle"), "Login OK, pero la respuesta no trae un ID válido.");
        return;
      }
