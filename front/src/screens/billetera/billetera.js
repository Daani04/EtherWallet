...
const Billetera = (props) => {
  ...
  const [redActiva, setRedActiva] = useState("Ethereum");
  const [direccion, setDireccion] = useState("0xA1b2...C9D0");
  ...
  const copiarDireccion = async () => {
    try {
      if (Platform.OS === "web") {
        ...
        Alert.alert("No disponible", "Tu navegador no permite copiar automáticamente.");
        return;
      }
      Alert.alert("Copiar", "Si queréis copiar de verdad en móvil, se conecta con expo-clipboard.");
    } catch (e) {
  ...
  const cambiarRed = () => {
    if (redActiva === "Ethereum") {
      setRedActiva("Polygon");
      setDireccion("0xP0ly...1234");
      return;
    }
    if (redActiva === "Polygon") {
      setRedActiva("BSC");
      setDireccion("0xB5c...88AA");
      return;
    }
    setRedActiva("Ethereum");
    setDireccion("0xA1b2...C9D0");
  };
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {/* ✅ FIX SCROLL: ScrollView con flex:1 + paddingBottom para que Nav no lo tape */}
      <ScrollView
        style={styles.scroll}
        ...
        <View style={[styles.container, dyn.container]}>
          ...
          <View style={styles.depositCard}>
            <View style={styles.depositTop}>
              <View style={{ flex: 1 }}>
                <Text style={styles.depositLabel}>Red</Text>
                <Text style={styles.depositValue}>{redActiva}</Text>
              </View>

              <Pressable onPress={cambiarRed} style={styles.secondaryBtn}>
                <MaterialIcons name="swap-horiz" size={18} color={COLORS.textMuted} />
                <Text style={styles.secondaryBtnText}>Cambiar</Text>
              </Pressable>
            </View>

            <View style={styles.depositDivider} />

            <Text style={styles.depositLabel}>Dirección</Text>

            <View style={styles.addressRow}>
              <Text style={styles.addressText}>
                {ocultarSaldo ? "••••••••••••••••" : direccion}
              </Text>

              <Pressable onPress={copiarDireccion} style={styles.copyBtn}>
                <MaterialIcons name="content-copy" size={18} color={COLORS.textMuted} />
              </Pressable>
            </View>

            <Text style={styles.depositHint}>
              Envía solo fondos en la red seleccionada para evitar pérdidas.
            </Text>
          </View>
                  ...
                  return (
                    <View key={m.type + "-" + index} style={styles.movRow}>
                      <View style={styles.movLeft}>
                        ...
                        <View style={{ flex: 1 }}>
                          {/* ✅ FIX: Value y Status ya NO se pisan */}
          ...
          {/* Extra para que Nav no tape y para que SIEMPRE puedas bajar */}
...
const styles = StyleSheet.create({
  ...
  scroll: { flex: 1 },
  // ✅ FIX SCROLL: paddingBottom grande + flexGrow
  scrollContainer: {
    flexGrow: 1,
  ...
  depositCard: { backgroundColor: COLORS.inputBg, borderWidth: 1, borderColor: COLORS.border, borderRadius: 20, padding: 16 },
  depositTop: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  depositLabel: { color: COLORS.textMuted, fontSize: 12, fontWeight: "800", textTransform: "uppercase", letterSpacing: 0.6 },
  depositValue: { color: "#fff", fontSize: 16, fontWeight: "900", marginTop: 6 },
  depositDivider: { height: 1, backgroundColor: "rgba(59,84,69,0.7)", marginVertical: 14 },
  addressRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 },
  addressText: { color: "#fff", fontSize: 14, fontWeight: "800" },
  copyBtn: {
    width: 40,
    height: 40,
    borderRadius: 14,
  ...
  depositHint: { marginTop: 10, color: COLORS.textMutedSoft, fontSize: 13, lineHeight: 18 },
  ...
  // ✅ FIX OVERLAP: filas bien separadas
